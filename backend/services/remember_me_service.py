...
class RememberMeService:
    ...
    def get_username_for_device(self, device_fingerprint: str) -> Optional[str]:
        """
        Get the username associated with a device fingerprint.

        Args:
            device_fingerprint: Device identifier to check

        Returns:
            Username if a token exists for this device, None otherwise
        """
        try:
            response = self.supabase.table(self.table_name).select('username').eq('device_fingerprint', device_fingerprint).limit(1).execute()
            if response.data and len(response.data) > 0:
                return response.data[0].get('username')
            return None
        except Exception as e:
            debug_log(f"Error getting username for device: {str(e)}", "bot_logic")
            return None

    def verify_device_for_recovery(self, device_fingerprint: str, username: str = None) -> Optional[Tuple[str, str]]:
        """
        Verify device fingerprint for recovery auto-login.
        This allows auto-login when token is missing from localStorage but credentials exist in Supabase.
        
        Note: This is less secure than token-based verification but allows recovery.
        We can't decrypt the password without the original token, so we return None for password.
        The caller should handle passwordless login or prompt for password.
        
        Args:
            device_fingerprint: Device identifier
            username: Optional username to verify matches device fingerprint

        Returns:
            Tuple of (username, None) if device verified, None otherwise
            Password is None because we can't decrypt without the original token
        """
        try:
            # Get the record by device fingerprint
            response = self.supabase.table(self.table_name).select('username').eq('device_fingerprint', device_fingerprint).limit(1).execute()
            
            if not response.data or len(response.data) == 0:
                debug_log(f"No token found for device fingerprint: {device_fingerprint[:16]}...", "bot_logic")
                return None
            
            stored_username = response.data[0].get('username')
            
            # If username provided, verify it matches
            if username and stored_username != username:
                debug_log(f"Username mismatch for device recovery: stored={stored_username}, provided={username}", "bot_logic")
                return None
            
            debug_log(f"Device verified for recovery: {device_fingerprint[:16]}..., username: {stored_username}", "bot_logic")
            
            # Return username but password is None (can't decrypt without original token)
            # The frontend/backend will need to handle passwordless login or prompt for password
            return (stored_username, None)
            
        except Exception as e:
            debug_log(f"Error verifying device for recovery: {str(e)}", "bot_logic")
            return None
